...
3. **Adventure LLM** - Creates narrative text (EXACTLY 2-4 sentences) with quest progression and full adventure history context
...
- **Narrative consistency system** - Adventure LLM receives full adventure history as compressed encounter summaries (max ~1200 chars per adventure)
- **Strict length enforcement** - Adventure LLM constrained to EXACTLY 2-4 sentences with explicit examples
...
DunGen uses a three-tier approach to maintain coherent, non-repetitive narratives:

**1. Encounter Summary Tracking:**
- `AdventureProgress.encounterSummaries` array stores one-sentence summary of each encounter
- Generated by `generateEncounterSummary()` which compresses narratives to ~100 chars
- Smart summarization: monster encounters → "Encountered [monster name]", NPCs → "Met [NPC name]", traps/puzzles → generic descriptions
- Max 12 summaries per adventure (7-12 encounters) = ~1200 characters total

**2. Full Adventure History Context:**
- `buildAdventureHistory()` joins all summaries with " → " separator
- Passed to Adventure LLM in every turn: "Adventure so far: [summary1] → [summary2] → [summary3]..."
- Provides complete story context without unbounded prompt growth
- Enables LLM to reference past events, avoid repetition, and maintain continuity

**3. Strict Length Enforcement:**
- Adventure LLM constrained to EXACTLY 2-4 sentences, NO MORE
- Explicit good/bad examples in instructions showing proper brevity
- Prevents rambling, repetitive descriptions
- Forces focus on most important narrative details

**Benefits:**
- Consistent narrative across all encounters in an adventure
- Logical progression and causality between encounters
- No contradictions in location details or NPC behavior
- Reduced repetition through awareness of past descriptions
- Maintains immersion through coherent storytelling
- Bounded context size (resets per adventure)
...
- `AdventureProgress` - includes `questGoal` field and `encounterSummaries` array (one-sentence summaries of each encounter for narrative continuity)
